...
class Backend {
	...
	if($_SERVER['SERVER_NAME']=='floortec.com'){
		mysql_connect('localhost', 'root', '') or die("Can't connect to server");
	       mysql_select_db('floortec') or die("Can't connect database");
	}else{
		mysql_connect('localhost', 'floortec_ft', 'pwft@123') or die("Can't connect to server");
	        mysql_select_db('floortec_ft1') or die("Can't connect database");	
	}
    ...
    function updateText($id,$text_vi,$text_en) {

        $time = time();

        $sql = "UPDATE text

                    SET text_vi = '$text_vi' , text_en = '$text_en'
    ...
    function insertText($text_vi, $text_en){

        try{

            $time = time();

            $sql = "INSERT INTO text VALUES(NULL,'$text_vi','$text_en')";
    ...
    function getListProduct($category_id=-1) {
        try{
            $arrReturn = array();
            $sql = "SELECT * FROM product WHERE category_id = $category_id OR $category_id = -1 ORDER BY id DESC ";                                 
            $rs = mysql_query($sql) or die(mysql_error());            
            while($row =mysql_fetch_assoc($rs)){
                $arrReturn[$row['id']] =  $row;                           
            } 
