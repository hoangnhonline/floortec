...
class Home {
   ...
   function __construct() {
	if($_SERVER['SERVER_NAME']=='floortec.local'){
    ...
    function getProductId($product_alias, $lang ="vi"){
        $sql = "SELECT id FROM product WHERE alias_vi = '$product_alias'";
        $rs = mysql_query($sql);
        $row = mysql_fetch_assoc($rs);
        return $row['id'];
    }
    ...
    function getListProduct($cate_id = -1) {
        $arrReturn = array();
        try{            
